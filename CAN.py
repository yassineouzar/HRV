...
def Xception():
    ...
    img_input = Input(shape=(50, 120, 160, 3))
    ...
    x = Dropout(0.2)(x)
    x = Dense(50, activation='linear')(x)##sortie à changer
    inputs = img_input
...
history = model.fit(train_data, epochs=100,
                                  steps_per_epoch= len(train_data.filenames) // 150,
                                  validation_data=train_data, validation_steps=len(train_data.filenames) // 150,
