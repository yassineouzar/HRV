...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
        ...
        for filename in batches_hr:
            ...
            Y = signal.resample(sig_liss, int(round(samps)), domain='time')
            #
            # plt.plot(Y)
            # plt.show()
    ...
    def next(self):
        ...
        #batches55 = batches55/np.linalg.norm(batches55)
        batch_y = np.array(batches55)
        #plt.plot(batch_y[0])

        batch_y = batch_y/np.linalg.norm(batch_y)
        # plt.plot(batch_y[0])
        pulse_pred = detrend(np.cumsum(batch_y), 100)

        # Peaks and Valleys detection
        indexes_peaks, _ = signal.find_peaks(-pulse_pred, distance=50)
        indexes_valleys, _ = signal.find_peaks(pulse_pred, distance=50)

        fps = 25

        time = np.arange(0, ((len(pulse_pred) / fps) - 1 // fps), 1 / fps)


        iHR_peaks = np.gradient(time[indexes_peaks])
        iHR_valleys = np.gradient(time[indexes_valleys])

        pxx, frequency = plt.psd(pulse_pred, NFFT=len(pulse_pred), Fs=25, window=np.hamming(len(pulse_pred)))

        lf_band = [0.04, 0.15]
        hf_band = [0.15, 0.40]

        lf_indexes = np.logical_and(frequency >= lf_band[0], frequency < lf_band[1])
        hf_indexes = np.logical_and(frequency >= hf_band[0], frequency < hf_band[1])

        lf = np.trapz(y=pxx[lf_indexes], x=frequency[lf_indexes])
        hf = np.trapz(y=pxx[hf_indexes], x=frequency[hf_indexes])

        print(lf, hf, lf/hf)
        # print(calc_rmssd(iHR_peaks))
        # iHR_peaks = 60 / iHR_peaks
        # iHR_valleys = 60 / iHR_valleys
        #print(iHR_peaks, iHR_valleys, extract_features.get_time_domain_features(iHR_peaks))
        #print(extract_features.get_frequency_domain_features(iHR_peaks))
        #print(extract_features.get_frequency_domain_features(iHR_valleys))
...
if __name__ == '__main__':
    ...
    batch_size = 1
    train_data = datagen.flow_from_directory(directory='E:/Emotion/ROI_BP4D', label_dir='G:/HR_BP4D_save_djamal',
                                             target_size=(120, 160), class_mode='label', batch_size=1,
                                             frames_per_step=1000, shuffle=False)
    print("finished1")
