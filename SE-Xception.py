...
#from Generator_yuv import ImageDataGenerator
...
train_data = datagen1.flow_from_directory(directory='/media/ouzar1/Seagate Expansion Drive/ROI_train_BP4D', label_dir='/media/ouzar1/Seagate Expansion Drive/HR_train_BP4D', target_size=(120, 160), class_mode='label', batch_size=batch_size, frames_per_step=50, shuffle=False)
test_data = datagen1.flow_from_directory(directory='/media/ouzar1/Seagate Expansion Drive/ROI_train2', label_dir='/media/ouzar1/Seagate Expansion Drive/HR_train2', target_size=(120, 160), class_mode='label', batch_size=14, frames_per_step=50, shuffle=False)
print("finished1")
from keras.models import Model
...
def Xception():
    ...
    x = Activation('relu')(x)
    x = Conv3D(32, (3, 3, 3), use_bias=False)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
...
checkpoint = ModelCheckpoint('weights.{epoch:02d}-{val_root_mean_squared_error:.4f}.h5',
                             ...
                             verbose=1, save_best_only=False)
history_checkpoint = CSVLogger("SE-Xception.csv", append=True)
...
history = model.fit(train_data, epochs=50,
                                  steps_per_epoch= len(train_data.filenames) // 1550,
                                  validation_data=test_data, validation_steps=len(test_data.filenames) // 700,
