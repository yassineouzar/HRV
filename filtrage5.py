...
def get_roi(image):
    ...
    image1 = np.resize(image,(120,160,3))
...
def append_to_list_hr(path_y, path_save, path_im, path_review, path_save_im):
    ...
    for i in range(50, len(list_dir)):
        ...
        for j in range(0,len(list_dir1)):
            ...
            blood_pressure = [filename for filename in list_dir2 if filename.startswith("BP_mmHg")]
            ...
            if s == '1':
                ...
                c = input("peaks or valleys: ")
                if c == 'p':
                    a, b = input("Enter boundes: ").split()
                    print(hr_p[int(a):int(b)])
                    hr_p = hr_p[int(a):int(b)]
                    ...
                    print(len(hr_p), len(hr_v))
                    print(heart[int(a):int(b)])
                    hr_v = hr_v[int(a):int(b)]
                    imgs = imgs[int(a):int(b)]
                    imgs_save = imgs_save[int(a):int(b)]
                    print(imgs[int(a):int(b)])
                    print(len(imgs))
...
path_save = '/media/ouzar1/Seagate Expansion Drive/HR_train_BP4D'
path_save_im = '/media/ouzar1/Seagate Expansion Drive/ROI_train_BP4D'

path_review = '/media/ouzar1/Seagate Expansion Drive/HR_train_review_BP4D'
sampling_rate = 1000
